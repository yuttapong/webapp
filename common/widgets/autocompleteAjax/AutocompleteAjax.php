...
class AutocompleteAjax extends InputWidget
{
    ...
    public function run() {
        ...
        if ($this->type=='key_name' && $this->multiple==true) {
        	$value = $this->model->{$this->attribute};
	        $optin= array_merge($this->options, [ 'class' => 'form-control'
	        		,'data-url'=>$this->getUrl()
	        		 
	        ]);
	        return   Html::activeHiddenInput($this->model,  "[{$this->rowIndex}]{$this->attribute}" , [ 'class' => 'form-control'])
	        . ($value ? Html::tag('div', "<img src='{$this->registerActiveAssets()}/images/load.gif'/>", ['class' => 'autocomplete-image-load']) : '')
	        . Html::activeTextInput($this->model, "[{$this->rowIndex}]{$this->attribute_name}" ,$optin);
        }else{
        	$optin= array_merge($this->options, [ 'class' => 'form-control'
        			,'data-url'=>$this->getUrl()
        	
        	]);
        	return  Html::textInput($this->name, '', $optin) ;
        }
       /* if ($this->multiple) {
