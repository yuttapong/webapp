...
<div class="kv-detail-heading">
    ...
    <?php endif; ?>
    <div class="kv-detail-crumbs" style="font-size: 14px;"><?= $name ?></div>
    ...
    <?php if ($showIDAttribute): ?>
        ...
        <div class="row"> 
            ...
            <div class="4">
                ...
                <?= $form->field($node, 'position_id')->widget(\kartik\widgets\Select2::className(), [
            'data' =>$node->getPositionDropdown(),
            'language' => 'th',
            'options' => [
   					'onchange '=>'$("#pos_name").val(this.options[this.selectedIndex].text);',
            		'placeholder' => 'Select a state ...'] + $inputOpts,
            'pluginOptions' => [
                'allowClear' => true

            ],      
        ]) ?>
                <?= $form->field($node, $nameAttribute,['inputOptions' => ['id' => 'pos_name']])->hiddenInput($inputOpts) ?>
            </div>
        </div>
    <?php else: ?>
        <?= $keyField ?>
...
<div class="row">
    ...
    <div class="col-sm-12">
        <?= $form->field($node, 'user_id')->widget(\kartik\widgets\Select2::className(), [
            'data' =>$node->getPersonnelDropdown(),
            'language' => 'th',
            'options' => ['placeholder' => '--พนักงาน--', 'id' => 'tree-user_id'] + $inputOpts,
            'pluginOptions' => [
                'allowClear' => true
            ],
            'pluginEvents' => [
                "select2:select" => "getCurrentSiteUser",
            ],
        ]) ?>
    </div>
</div>
<div class="well">
<?php
echo $form->field($node, "sites")->checkboxList($node->getSiteDropdown(), [
            'prompt' => '--ไซต์งาน--',
            'multiple' => true,
        ]+$options);
?>
...
<?= $form->field($node, 'company_id')->hiddenInput(['value' =>''])->label(false) ?>
<!--//////////////////////////////////////////////////////////////////-->
...
<?php
$js = <<<'EOD'
$("#treemanager-company_id").val($("#current-company-id").val());
function getCurrentSiteUser(){
    var user_id = $("#tree-user_id").val();
    if(user_id){
          $.ajax({
             url:'sites-user',
             ...
             dataType:'json',
             success:function(rs){
                  var el = $("input[name='TreeManager[sites][]']");
                  el.each(function(){
                     var site = $(this);
