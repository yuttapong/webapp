...
<?php if ($iconsList == 'text' || $iconsList == 'none'): ?>
    <?php if ($showIDAttribute): ?>
        <div class="row">
            ...
            <div class="col-sm-8">
                <?php //$form->field($node, $nameAttribute)->textInput($inputOpts) ?>
            </div>
        </div>
    <?php else: ?>
        <?= $keyField ?>
...
<div class="row">
    <div class="col-sm-12">
        <?= $form->field($node, 'user_id')->widget(\kartik\widgets\Select2::className(),[
            'data' => ArrayHelper::map(OrgPersonnel::find()->all(),'id','firstname_th'),
            'language' => 'th',
    ...
    <div class="col-sm-12">
        <?= $form->field($node, 'position_id')->widget(\kartik\widgets\Select2::className(),[
            'data' => ArrayHelper::map(OrgPosition::find()->all(),'_id','name_th'),
            'language' => 'th',
            'options' => ['placeholder' => 'Select a state ...'],
            'pluginOptions' => [
                'allowClear' => true
